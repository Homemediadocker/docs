...
- See the [Environment page](./getting-started/setup-env.mdx) for setting up your different env variables. **You will probably encounter errors when spinning up containers if you just copied them without configuring anything. especially with the [VPN setup](./home-media-containers/vpn.mdx) and the [Authentik SSO Setup](./authentik-sso.mdx)**
