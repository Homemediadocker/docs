...
## Containers that Depend on Network Shares Exiting (Code 128)

This can happen on newer machines that run much faster and can chew threw all the startup mess faster than the fstab can run to mount the drives. If this happens, there are a few options. You can bind the network share directly in docker compose files as a volume and reference it, or you can update the docker service to wait 20-30 seconds before spinning up the containers. 
...
This basically tells the docker service to count to 30 before it does anything. This fixed the issue for me. You can also try to wait for the network devices to become available but i found that to be flaky.
...
[Migrating Sonarr and Radarr from Windows 10 to unRAID](https://www.reddit.com/r/unRAID/comments/ibgone/migrating_sonarr_and_radarr_from_windows_10_to/) - This proved to be useful in my docker setup which runs on an Ubuntu 22.04 host machine.

## Traefik and Portainer Don't work properly

If Traefik and Portainer aren't functioning (check both), this could be related to the `/var/run/docker.sock` - In Docker 29.0.0, Earlier versions of Traefik and Portainer failed to function due to compatibility with the new Major version of Docker.

- Portainer will show the stack ans `UP` but when you try to click into the stack, it will fail to connect. 
- Traefik will not recognize any labelled containers in the stack.

### Traefik and Portainer Resolution
An issue was posted in [portainer/portainer/#12925](https://github.com/portainer/portainer/issues/12925) that was having this issue. [This comment](https://github.com/portainer/portainer/issues/12925#issuecomment-3516549977) was the fix.
